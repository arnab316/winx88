...
// //   // GET /games/my-bets?status=LOST
// //   // GET /games/my-bets?status=PLACED     ← pending/unsettled
...
// //     @Req() req: any,
// //     @Query('status')  status?:  string,
// //     @Query('gameId',  new DefaultValuePipe(0), ParseIntPipe) gameId:  number = 0,
// //     @Query('page',    new DefaultValuePipe(1),  ParseIntPipe) page:   number = 1,
// //     @Query('limit',   new DefaultValuePipe(20), ParseIntPipe) limit:  number = 20,
// //   ) {
// //     try {
// //       const validStatuses = ['WON', 'LOST', 'PLACED', 'CANCELLED'];
// //       const data = await this.gameService.getUserBetHistory(req.user.sub, {
...
// //         limit,
// //       });
// //       return { statusCode: HttpStatus.OK, message: 'Bet history', ...data };
// //     } catch (error: any) {
// //       throw new HttpException(
// //         { statusCode: error?.status || HttpStatus.INTERNAL_SERVER_ERROR,
...
//   // GET /games/my-bets?status=WON&gameId=1&page=1&limit=20
//   // User's personal bet history — requires JWT
//   @UseGuards(JwtAuthGuard)
//   @Get('my-bets')
//  async myBetHistory(
//     @Req() req: any,
...
//     try {
//       const data = await this.gameService.adminListGames();
//       return { statusCode: HttpStatus.OK, message: 'Games list', count: data.length, data };
//     } catch (error: any) {
//       throw new HttpException(
//         { statusCode: error?.status || HttpStatus.INTERNAL_SERVER_ERROR,
//           message: error?.message || 'Failed to fetch games list' },
...
//   // DELETE /games/admin/:id  (soft delete, or ?hard=true for hard delete)
//   @UseGuards(AdminGuard)
//   @Delete('admin/:id')
//   async deleteGame(
//     @Param('id', ParseIntPipe) id: number,
...
//         thumbnail_url: body.thumbnail_url,
//         display_category: body.display_category,
//         max_payout_per_round: body.max_payout_per_round ? Number(body.max_payout_per_round) : undefined,
//         round_code: body.round_code,
//         open_time: body.open_time, close_time: body.close_time, draw_time: body.draw_time,
//       });
//       return { statusCode: HttpStatus.CREATED, message: 'Game and round created', data: result };
//     } catch (error: any) {
//       throw new HttpException(
//         { statusCode: error?.status || HttpStatus.INTERNAL_SERVER_ERROR,
...
//   async adminCreateRound(@Body() body: any) {
//     try {
//       if (!body.game_id || !body.round_code || !body.open_time || !body.close_time || !body.draw_time) {
//         throw new BadRequestException('game_id, round_code, open_time, close_time, draw_time are required');
//       }
//       const result = await this.gameService.adminCreateRound({
//         game_id: Number(body.game_id), round_code: body.round_code,
//         open_time: body.open_time, close_time: body.close_time, draw_time: body.draw_time,
//       });
//       return { statusCode: HttpStatus.CREATED, message: 'Round created', data: result };
//     } catch (error: any) {
//       throw new HttpException(
//         { statusCode: error?.status || HttpStatus.INTERNAL_SERVER_ERROR,
//           message: error?.message || 'Failed' },
...
//    // This is the betting lobby endpoint.
//   @Get('active-rounds')
//   async allActiveRounds(
//     @Query('digitLength', new DefaultValuePipe(0), ParseIntPipe) digitLength: number,
//     @Query('gameId',      new DefaultValuePipe(0), ParseIntPipe) gameId:      number,
//   ) {
//     try {
//       const data = await this.gameService.getAllActiveRounds({
//         digitLength: digitLength > 0 ? digitLength : undefined,
//         gameId:      gameId      > 0 ? gameId      : undefined,
//       });
//       return {
...
//   //
//   // Public — no auth needed.
//   // Returns only games that currently have an open round.
//   // Cleaner than GET /games for the "play now" lobby — no dead games shown.
//   @Get('with-active-rounds')
//   async gamesWithActiveRounds() {
//     try {
//       const data = await this.gameService.getGamesWithActiveRounds();
//       return {
//         statusCode: HttpStatus.OK,
//         message:    'Games with active rounds',
//         count:      data.length,
//         data,
//       };
//     } catch (error: any) {
...
export class GameController {
  ...
  async gamesWithActiveRounds() {
    try {
      const data = await this.gameService.getGamesWithActiveRounds();
      return { statusCode: HttpStatus.OK, message: 'Games with active rounds', count: data.length, data };
    } catch (error: any) {
      throw new HttpException(
        { statusCode: error?.status || HttpStatus.INTERNAL_SERVER_ERROR,
          message: error?.message || 'Failed to fetch games' },
