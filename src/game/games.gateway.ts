...
export class GamesGateway implements OnGatewayConnection, OnGatewayDisconnect {
   ...
   */
  emitHotNumbersUpdated(payload: {
    gameId: number;
    action: 'added' | 'deleted' | 'toggled' | 'expired';
    hotNumbers: { id: number; number: string; expires_at?: string; seconds_until_expiry?: number }[];
  }) {
    const room = `game:${payload.gameId}`;
    const event = payload.action === 'expired' ? 'hotnumbers:expired' : 'hotnumbers:updated';
    this.server.to(room).emit(event, {
      ...payload,
      announcedAt: new Date().toISOString(),
    });
    this.logger.debug(`Emitted ${event} to ${room} (action: ${payload.action})`);
  }
}